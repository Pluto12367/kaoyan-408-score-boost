...
- `pdf-lib` 1.17.1 reference: https://pdf-lib.js.org/docs/api/classes/pdfdocument. The provider split artifact uses `PDFDocument.load`, `PDFDocument.create`, `copyPages`, and `save` to create a real 1-based inclusive page-range PDF without an external process. The qpdf worker planned for Task 8 can replace this internal implementation without changing the split-key contract.
