...
- P0：知识点目录接入学习引擎（P0-1）已完成：DB 知识点参与掌握度/薄弱/推荐/计划，`POST /knowledge-points` 持久化，空库/已导入库两种启动顺序均已覆盖（ROADMAP 状态“已完成”，2026-08-05）。剩余项为全量知识目录（`KnowledgeNode` 1149 原子点）与经典闭环的统一（P0-2，方案待确认，见设计文档）。
