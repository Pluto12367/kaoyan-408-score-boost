...
- 掌握度：两套口径并存——经典闭环（`getMasteryMap`/薄弱报告/推荐/计划）基于 `KnowledgePoint` 目录实时计算（DB 存在时用导入的 16+ 知识点，无库回退内置 4 点；P0-1 已完成）；“今日提分”引擎基于 `KnowledgeNode` 全量目录 + `UserKnowledgeMastery` 持久化。口径统一方案见 P2-2。
...
- P0：知识点目录接入学习引擎（P0-1）已完成：DB 知识点参与掌握度/薄弱/推荐/计划，`POST /knowledge-points` 持久化，空库/已导入库两种启动顺序均已覆盖（ROADMAP 状态“已完成”，2026-08-05）。P0-2（全量目录与经典闭环统一）按方案 B 实施中：映射/命名/接线已完成并部署；方案 C Phase 1（历史记录回填 `UserKnowledgeMastery` + 题库清重）与阶段 1（知识图谱掌握度着色，`GET /knowledge/mastery` + 节点状态徽章 + 详情抽屉“我的掌握度/去练习”）已完成，待提交部署。
- P1：会话模式考试不写评估历史；全新库"先导入后启动"有启动崩溃风险；AI 答疑模板化 + 硬编码；题库内容正式入库流程待固化。
- P2：掌握度两套口径；StudyService/App.tsx 单文件过大；多知识点题目只记首点；演示模式标识不醒目。（评估历史/试卷/系统配置转正式表已完成）
