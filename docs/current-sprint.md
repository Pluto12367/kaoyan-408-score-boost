...
- **进度**：**V12-M1 Evidence Foundation ✅**（EB-1/EB-2 闭合：活动/证据/能力三层语义 + `EVIDENCE_RECORDED` 证据账本 + `GET /coach/learning-evidence`，零迁移，35 项新测试）；**V12-M2a Recommendation Exposure ✅**（EB-3 闭合：曝光遥测 + `GET /coach/recommendation-funnel` 只读漏斗，`exposed=null≠0`，28 项新测试）。**门禁状态：AGENTS.md §7 全部可执行并通过**（`npm test` 2112/2110/0/2、`build:api`、`build:web` 均 exit 0）——沙箱 spawn EPERM 替代门禁已不再必要。历史 LE/V10 线：F1 真题对标（M1-M5 关闭）✅；F2 模考诊断（后端+前端）✅；F3 遗忘防线 M1 影子基线 ✅；F4 大题采分点 content-blocked。V11 M1-M4 全部 code-complete 且已推送 origin。
