...
> **2026-09-12 S1 Score Anchor 完成（SCORE LEDGER + SCALE/PROVENANCE 修复；所有者预批方向内自主实施，已提交推送）**：按所有者 DSH 自主实施指令完成 S1（实现范围严格限 S1，未触碰 S2/S3/S4/ROI/排序/Mastery 语义/V12-M3）。①**Score Ledger 四新表**（ScorePrediction/ScoreAssessment/ScoreOutcome/ScoreCorrection + User.examDate 一列，迁移 20260912120000 additive 零既有列改动，migrate diff 零漂移，真实部署测试库成功）；②**结构隔离**：prediction/assessment/outcome 三写路径三守卫，学生自称 TEACHER_GRADED=403 零写入，correction 追加式（白名单字段+原因+重算 normalized），证据行零 UPDATE，outcome verification 单向 unverified→verified 且仅教师/管理员（teacher 需授权记录）；③**量纲修复**：normalizeScore 拒绝缺 totalScale（废除 ||150 回退），semantic 区分 exam_total/accuracy_rate，in-app 整卷的“分数”诚实更名为正确率口径且**结构上不得与总分预测配对**——150vs100 混算（原 predicted=26 vs actual=96/100 error=70）从“可能的错误”变为“不可能的表达式”（shared buildScoreCalibration 兼容性强制 + legacy 路径 totalScore≠150 直接排除）；④**预测首次持久化**：报告页预测按账户+日幂等落库（modelVersion+inputsSnapshot 可考古），paper 提交 best-effort 双写 accuracy_rate 行；⑤**校准重构**：ScoreLedger 主路径（持久化预测×带来源证据，未验证 outcome 排除计入 pendingVerification）+ legacy 回退（id 前缀+totalScore 读时解释，零回填零改写），MAE/bias/median 按 provenance 分层**永不混合**，E1 预注册门禁（每组 n≥5 且 median<15，n=5 过/median=15 不过）+ 学生四态诚实状态机；⑥**API/UI**：/coach/score-evidence 六端点（401 守卫集成断言）+ ScoreAnchorPanel（预测/成绩视觉隔离、来源徽标、无锚态不伪造 0 分、静态演示 return null）。**门禁**：npm test 2374/2372/0/2 exit 0（基线 2324/2322/0 +50 零回归）、build:api/web exit 0、**test:integration:score-anchor（新）10 步 ALL PASS**（真实 HTTP+PostgreSQL：幂等/边界/教师授权验证/逐行 scalePair=150/150/correction 折叠）、score-loop 16 环 ALL PASS（校准行 26/96/70→28/96/68 合法量纲）、effectiveness/event-key exit 0。**有理由的既有断言更新**：score-calibration-service 夹具补 id 字段（来源推导所需）+ 2 个既有测试语义随缺陷修复更新，另加 3 个 S1 新测试。**已知限制**：ScoreLossItem 延后至 S3 前、生产校准数字=0（E1 待真实学生与教研判分，诚实缺席）、预测同日首值优先。文档：docs/score-anchor-design.md / score-ledger.md / score-calibration.md。**S1 COMPLETE ≠ 提分已证明；北极星仍是 Verified Score Gain / 30d。下一步候选（待所有者）：S2 Transfer Probe。**
...
> **2026-09-11 V12.1 修复已部署并复验通过 → `M3 REVIEW → MASTERY PRODUCTION INTEGRATION = VERIFIED`（生产闭环）**：所有者批准后在服务器**再次部署** `ffb3af85831da55c3b7060af901acf073736194c`（V12.1 节点解析修复）。**部署**：`FETCH_HEAD` = `ffb3af8` → 字面 SHA `switch --detach` → 落地检查 `question-node-resolution.ts` 存在 → `deploy.sh` 备份门禁（`kaoyan408-20260911T143349Z.dump`）→ 镜像重建（builder 12–14 重跑：`build:shared` 6.0s / `prisma generate` 4.1s / `build -w apps/api` 20.0s；gateway 全 CACHED 因前端无改动）→ 三容器 healthy → **`postgres` 仍 `Up 5 days`（数据卷保留）**。**硬门禁全过**：`/health` 200 + `dataSource:postgresql`、`migrate status` 36 up to date、`ReviewAttempt` 四列 nullable 无默认、`Question.rubric` jsonb YES、历史行 9/9 全 NULL、`MASTERY_SEMANTICS = unset` + 启动日志 legacy、6 端点全 401。**决定性复验（对 `q-003` —— 修复前 100% 失效的那一类"只有 legacy 桥接、无直接标注"的题）**：日志由 `no_knowledge_node` 变为 **`Review mastery projection applied`**；`CO-C03-S05-P01` 的 `mastery = 0.9258780201143995`、`version = 49`、`lastLearnedAt = 2026-09-11T14:41:19.704Z` **与复习响应的 `lastReviewedAt` 毫秒级一致**；全库 `REVIEW_MASTERY_APPLIED` **恰好 1 条且 `semantics = legacy`**（C1 OFF 在新路径上同样成立）；**生产幂等实测**：同幂等键重发返回**已持久化的 attempt**（`reviewedAt` 与首次毫秒级相同）、`mastery/version` 均未变化；**学生隔离实测**：同节点的另一名学生（`version 0`、`lastLearnedAt 2026-08-20`）未受影响。**修复覆盖的是全部 332 道题**（非单题特例）。**一项未在生产侧直接复验（如实记录）**：`priority/opportunity/recommendation` 三条下游腿需 teacher/admin 令牌读 `/coach/shadow-decision-chain`，本次未在生产执行——已在构建机真实 HTTP + PostgreSQL 六案例 E2E（含真实 Δpriority/Δopportunity/Δrank）与 15×6 队列验证，且生产侧权威掌握度已确认更新。**判定**：`M3 PRODUCTION DEPLOYMENT = VERIFIED`，P0 = 0、P1 = 0；`NEW REGRESSION = 0`（`npm test` 2324/2322/0/2、`build:api`/`build:web` exit 0、7 集成套件全 exit 0）。报告 §16.7/§16.9 已更新为最终判定与完整部署记录；账本另记：`1234@qq.com` 密码曾进入服务器 shell 历史，操作员已执行 `history -c`。**遗留 Owner 决策项不变**：`QuestionKnowledgeNodeTag`（B13，332 题 0 直接标注）仍需作为**独立内容任务**处理，但**已不再阻塞功能**（代码可走桥接）；服务器 `remote.origin.fetch` 单分支 refspec 是"陈旧 161 提交"的根因，Runbook CHECKPOINT 3b 已给出修复命令。

> **2026-09-11 V12.1 M3 生产部署 + 真实 smoke 抓到并修复一处真实缺陷（节点解析缺回退层）**：所有者批准部署 → 人工在腾讯云服务器执行。**部署过程本身全部通过**：`git fetch` 后**发现服务器 remote-tracking ref 陈旧 161 个提交**（`git fetch origin <branch>` 只写了 `FETCH_HEAD`，未更新 `origin/<branch>`，读出来是 2026-08-31 的 `a0de9ee`）→ 改用**字面 SHA** `switch --detach` 绕开；回滚点记录为 **`32ab233`**（V11-M1，有效祖先，落后 38 提交，据此更正此前 UNVERIFIED 的"生产原提交"）；`deploy.sh` 备份门禁生效（`kaoyan408-20260911T135052Z.dump`）；`postgres` **未被重建**（`Up 5 days`，数据卷保留）。**部署后硬门禁全过**：app healthy、`/health` 200 + `dataSource:postgresql`、`migrate status` **36 个迁移** up to date（本次实际落地 **2 个**迁移：`question_rubric` + `review_attempt_schedule_metadata`，**34 → 36**，非此前假设的 35 → 36）、`ReviewAttempt` 四列 nullable 无默认、`Question.rubric` = jsonb YES、**历史行 9/9 全 NULL**、`MASTERY_SEMANTICS = unset` + 启动日志 `legacy`（C1 OFF）、6 个端点**全部 401**。**但真实业务 smoke 抓到一处真实缺陷**：六环 SQL 显示 `evidence_receipt` 有值且 `id_matches_attempt = t`（**M3-A 事件保真在生产生效**），而 **`mastery_claim` 为空** → 日志 `Review mastery projection no_knowledge_node`。**根因两层**：①**既有数据缺口**——生产 `QuestionKnowledgeNodeTag` **全库 0 行**（332 题一个都没有直接节点标注），全部靠 legacy 桥接 `QuestionKnowledgePoint` → `KnowledgePointNodeMap`（16 条 PRIMARY）解析；仓库无任何 seed 写该表，且 V11-M1 的 `/admin/data-quality` **本就在观测此缺口**（B13）→ **非本次部署引入**；②**本报告的实现缺陷**——`ReviewMasteryIntegrationService.resolvePrimaryNode` 只查了 tag 表单层，而生产 `resolveKnowledgeNodesForQuestion` 有**三层**（可信直接标注 → 桥接标注 → **`QuestionKnowledgePoint`→`KnowledgePointNodeMap` 回退链**），缺第 3 层 ⇒ 解析 null ⇒ 投影按设计**安全拒绝**（拒绝而非写错数据）。**影响不对称**：练习路径（三层）一直正常，只有复习路径不动——根因是**我在本应复用同一口径处引入了第二套口径**。**为何本地全绿却没抓到（入册教训）**：三个 E2E/队列脚本共 **8 处手工 `questionKnowledgeNodeTag.create`**，E2E 跑在**比生产更干净的数据形状**上——*真实数据库 + 真实 HTTP ≠ 真实数据形状*。**拒绝了一个错误的替代方案**：另一会话提出"执行数据迁移填充 332 行标签"，被否——①用数据写入修代码缺陷是错的工具（代码修完后同样生效且零写入）②只补数据无法覆盖**未来新增题目**（回退层仍必需）③16 条映射覆盖 332 题 ≈ 平均 1 条对 20 题，把**粗粒度推导**写进**逐题精确表**等于让启发式冒充人工标注 ④会**抹掉** `/data-quality` 的 B13 指标（用插入让指标变绿）。**修复（V12.1，所有者批准）**：新增 `apps/api/src/study/question-node-resolution.ts` 的 `resolvePrimaryNodeByQuestion`（快路径一次批量查 tag；**仅对 tag 表答不出的题**回退到生产解析器；PRIMARY 优先收敛于此），**4 处**统一改用它——①`review-mastery-integration.service.ts` 的 `resolvePrimaryNode`（根因）②`review-mastery-shadow.service.ts` 的 `getShadow` ③④`review-semantics-shadow.service.ts` 的 `assembleReplayInputs` + `getShadow`。**第二处同根因缺陷是夹具修正后立刻暴露的**：cohort 脚本报 `all questions must resolve to a node (1 !== 0)` —— 两个影子服务也只查 tag 表，意味着**在 332 题 0 标注的生产库上两个影子端点对 100% 真实复习都是盲的**（`eventsWithoutNode = 1`）。**未改动**：`applyReview`、`ScoreCenter` 写入语义、C1 开关、Prisma schema、迁移、F4。**夹具同步修正**：两个 E2E/队列脚本改为**桥接形状**（= 真实生产形状），Case B 保留 `direct` 作对照 → 两条路径均端到端覆盖；过程中修掉两个自造缺陷（cohort 脚本重复建 `KnowledgePoint` 致唯一约束冲突；测试库 27 个残留点污染后续运行）。**修复后复验**：`npm test` **2324/2322/0/2 exit 0**（+3 新测试，零回归）、`build:api`/`build:web` exit 0、**7 个集成套件全部 exit 0**；桥接形状下 **生产 E2E：10 attempts → 10 markers，六案例 A–F 全过，Case D 幂等 1/1/1**；**cohort：authoritative mastery changed on 15/15 reviewed nodes（修复前 0/15）、31 attempts → 31 occurrence-keyed receipts (1:1)、31 mastery applications、authoritative writes = 0 across 6 tables**。**判定修正**：原报告 READY 判定**每一项本地结论仍成立**，但"PostgreSQL E2E PASS"被夹具形状放大了覆盖面 → 修正为 **READY（代码层，V12.1 修复后）/ 线上部署 = NOT VERIFIED（待重新部署复验 `mastery_claim` 非空）**；线上 `c7d9093` 处于**安全状态**（拒绝而非写错，C1 OFF，基础设施与迁移正常）但 M3 功能目标未达成。**新增 Owner 决策项**：是否补齐 `QuestionKnowledgeNodeTag`（B13 缺口）——须作为**独立内容任务**（dry-run → 写 provenance 使 `direct`/`bridge` 可区分 → 精确记录插入集可回滚 → 表述为"推导"而非"已核实"）。报告补 §16 追加章节：`docs/v12-m3-review-mastery-production-report.md`；Runbook 同步：`docs/manual-production-deployment-runbook-v12.1.md`（字面 SHA 取代 `origin/<branch>`、迁移数 34→36、`mastery_claim` 为空的判定与处置、节点解析背景）。
