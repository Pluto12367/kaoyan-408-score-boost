...
- **进度**：**V12-M1 Evidence Foundation ✅**（EB-1/EB-2 闭合）；**V12-M2a Recommendation Exposure ✅**（EB-3 闭合）；**V12-M2b Learning Evidence Ledger ✅**（证据层前端兑现）；**V12-M3 Review Semantics 审计 + 影子 ✅**（EB-5 精确化：`applyReview` 不改 mastery、`retention` 恒为 1；矩阵 + 只读影子已交付，**Phase C 切换待批准**）。**门禁状态：AGENTS.md §7 全部可执行并通过**（`npm test` 2142/2140/0/2、`build:api`、`build:web` 均 exit 0）。下一步：**V12-M4 Score Opportunity Shadow**、V12-M5 Real Score Calibration、F4 大题训练。历史 LE/V10 线：F1 真题对标 ✅；F2 模考诊断 ✅；F3 遗忘防线 M1 影子基线 ✅；F4 content-blocked。V11 M1-M4 全部 code-complete 且已推送 origin。
