...
- **进度**：**V12-M1 Evidence Foundation ✅**（EB-1/EB-2 闭合：活动/证据/能力三层语义 + `EVIDENCE_RECORDED` 证据账本 + `GET /coach/learning-evidence`）；**V12-M2a Recommendation Exposure ✅**（EB-3 闭合：曝光遥测 + `GET /coach/recommendation-funnel` 只读漏斗，`exposed=null≠0`）；**V12-M2b Learning Evidence Ledger ✅**（证据层前端兑现：报告总览三类证据可区分 + 系统"拒绝判断"对学生可见）。**门禁状态：AGENTS.md §7 全部可执行并通过**（`npm test` 2121/2119/0/2、`build:api`、`build:web` 均 exit 0）。下一步：**V12-M3 Review Semantics Unification（EB-2/EB-5，写语义变更，需所有者批准）**、V12-M4 Score Opportunity Shadow、V12-M5 Real Score Calibration。历史 LE/V10 线：F1 真题对标（M1-M5 关闭）✅；F2 模考诊断（后端+前端）✅；F3 遗忘防线 M1 影子基线 ✅；F4 大题采分点 content-blocked。V11 M1-M4 全部 code-complete 且已推送 origin。
