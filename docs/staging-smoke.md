...
该检查用于在邀请学生内测前，验证 GitHub Pages、Railway API 和 PostgreSQL 的关键链路仍然正常。

## 覆盖范围

1. `/health` 返回 PostgreSQL 已连接，并包含 `x-request-id`。
2. GitHub Pages 来源通过 CORS 预检，未知来源不被放行。
3. staging 环境的演示登录保持关闭。
4. 专用学生账号可以登录；账号不存在时首次运行会自动注册。
...
- Secrets：`STAGING_SMOKE_EMAIL`、`STAGING_SMOKE_PASSWORD`

专用账号仅用于自动检查，不邀请真实学生使用。密码至少 12 位，不能提交到仓库或写入日志。
...
成功时会输出八个 `PASS` 检查项和一份不含账号、密码、令牌的结果摘要。
