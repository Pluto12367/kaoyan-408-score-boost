 ✅（永空 tab 已移除，相关空态条件与断言同步清理） | ✅（死链已移除：TodayLearningRouteView 删除 onOpenPlan prop 与调整计划按钮；契约改反向断言） | ✅（AI 分区已加入侧边栏与底部导航（第六个 tab，6 列栅格）；mobile-nav 契约同步 5→6） |# V8 Master Backlog（55 项，证据支撑）
...
| 12 | 时长预算入口："我有 15/30/60 分钟"快速会话——后端开放 composeDailyPlan 四档引擎到经典闭环，前端首页加预算选择 | B7/U22 | ✅（GET /practice-sets/recommended?minutes= 预算参数：15→5 题/30→10 题/60+ 基准档；练习面板加快速会话按钮。score-center 引擎入口未动） |
...
| 29 | 错题行"做同考点变式"名实一致（直接进入变式练习） | U12 | ✅（wrong-row 做同考点变式直接启动变式练习，不再只开详情） |
| 30 | 删除"即将到期"空 tab 或实现 true 预到期窗口（未来 3 天） | U13 | ✅（永空 tab 已移除，空态条件与断言同步清理） |
...
| 37 | 题库 1/328 与预计时长自洽（按题组时长或显示"全天训练流"） | P14 | ✅（进度卡标注 题库推进 范围，与组训练预计时长区分口径） |
...
| # | 项 | 来源 | 状态 |
|---|---|---|---|
| 41 | ai-metrics 7 个未接线 record 方法接生产者或删除；暴露 GET /ai/learning-intelligence（admin） | T4 | ✅（新增 GET /ai/learning-intelligence 暴露 learning-intelligence 快照；7 个 record 方法的生产接线仍留待按需评估） |
...
| 51 | LearningTrend 柱高语义化（改为题量或时长，去合成权重） | U7 | ✅（趋势柱高 = 真实练习题数，标签改 每日练习题数） |
| 52 | InitializationCore 假等待改真实资源加载（或压缩至 <1.5s） | U21 | ✅（注册动画 6.6s 压缩至 3s） |
...
| 56 | 错题本与 canonical WrongQuestionReview 表漂移对账（23 vs 12）：数据来源审计 → resolved 回写策略 → 迁移评估（架构保护边界，设计先行） | P5/新 | ✅（设计已出：docs/v8-wrong-question-reconciliation-design.md，推荐 B 回填脚本 + C 写路径审计并行；实施待所有者确认 + 备份） |
