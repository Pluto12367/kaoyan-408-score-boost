...
export class DailyBriefController {
  ...
  /**
   * V12-M5 — real score calibration (EB-4, NON-AUTHORITATIVE).
   *
   ...
   * carries the prediction range, the facts it rested on, and the real score —   * never a merged number. Below the floor the calibration reports
  ...
  @UseGuards(RoleGuard)
  @Roles('teacher', 'admin')
  async getScoreCalibration(
    @CurrentUser() user: UserProfile,
    @Query('userId') viewUserId?: string,
  ) {
    const userId = this.resolveUserId(user, viewUserId);
    if (!this.scoreCalibration) {
      return { generatedAt: new Date().toISOString(), result: null, reason: 'store_unavailable' };
    }
    const result = await this.scoreCalibration.getCalibration(userId);
    if (result == null) {
      return { generatedAt: new Date().toISOString(), result: null, reason: 'store_unavailable' };
    }
    return { userId, ...result };
