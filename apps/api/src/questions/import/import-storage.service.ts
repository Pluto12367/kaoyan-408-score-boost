...
export class ImportStorageService {
  ...
  async putCandidateAsset(buffer: Buffer, mediaType: 'image/png' | 'image/jpeg' | 'image/webp'): Promise<{ storageKey: string }> {
    const extension = mediaType === 'image/png' ? 'png' : mediaType === 'image/jpeg' ? 'jpg' : 'webp';
    const id = randomUUID();
    const directory = resolve(this.config.temporaryDirectory, 'candidate-assets');
    await mkdir(directory, { recursive: true, mode: 0o700 });
    const path = resolve(directory, `${id}.${extension}`);
    await this.assertTrustedParent(this.config.temporaryDirectory, path);
    await writeFile(path, buffer, { flag: 'wx', mode: 0o600 });
    return { storageKey: `candidate-asset/${id}.${extension}` };
  }

  async resolveAssetPath(storageKey: string): Promise<string> {
    if (/^page-preview\/[a-f0-9-]{36}$/u.test(storageKey)) return this.resolvePagePreviewJpegPath(storageKey);
    const temporary = /^candidate-asset\/([a-f0-9-]{36}\.(?:png|jpg|webp))$/u.exec(storageKey);
    const permanent = /^permanent\/([a-f0-9]{2})\/([a-f0-9]{64}\.(?:png|jpg|webp))$/u.exec(storageKey);
    const path = temporary
      ? resolve(this.config.temporaryDirectory, 'candidate-assets', temporary[1])
      : permanent ? resolve(this.config.permanentDirectory, permanent[1], permanent[2]) : undefined;
    if (!path) throw new BadRequestException('Question import asset storage key is invalid');
    await this.assertTrustedParent(temporary ? this.config.temporaryDirectory : this.config.permanentDirectory, path);
    const metadata = await lstat(path);
    if (!metadata.isFile() || metadata.isSymbolicLink() || metadata.size <= 0) throw new BadRequestException('Question import asset is unavailable');
    return path;
  }

  async removeTemporaryAsset(storageKey: string): Promise<void> {
    if (!/^candidate-asset\/[a-f0-9-]{36}\.(?:png|jpg|webp)$/u.test(storageKey)) return;
    const path = await this.resolveAssetPath(storageKey);
  ...
  async copyToPermanent(storageKey: string, sha256: string, mediaType: string): Promise<string> {
    ...
    if (!extension) throw new BadRequestException('Question import asset type is invalid');
    const source = await this.resolveAssetPath(storageKey);
    const directory = resolve(this.config.permanentDirectory, sha256.slice(0, 2));
    await mkdir(directory, { recursive: true, mode: 0o700 });
    const target = resolve(directory, `${sha256}.${extension}`);
    await this.assertTrustedParent(this.config.permanentDirectory, target);
    try { await copyFile(source, target, 1); } catch (error: unknown) {
      if (!(error && typeof error === 'object' && 'code' in error && (error as { code?: string }).code === 'EEXIST')) throw error;
    }
    return `permanent/${sha256.slice(0, 2)}/${sha256}.${extension}`;
  }
  ...
  private async assertTrustedParent(expectedParent: string, path: string) {
    ...
    if ((parent !== actualParent && !isContained(parent, actualParent)) || !isContained(parent, resolve(path))) {
      throw new BadRequestException('Upload path escaped private storage');
    }
  }
