...
export function QuestionImportWorkspace() {
  ...
  useEffect(() => { void loadBatches().then(startPolling).catch((reason) => setError(reason instanceof Error ? reason.message : '无法读取导入批次')); return () => { mounted.current = false; stopPolling(); }; }, [loadBatches, startPolling, stopPolling]);
