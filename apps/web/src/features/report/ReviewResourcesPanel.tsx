...
export function ReviewResourcesPanel({ resources, onRetry }: { resources: ModuleResource<ReviewResourceRecommendation>; onRetry: () => void }) {
  if (!resources.data) return <ModuleUnavailable id="review-resources" title="复习资源" resource={resources} onRetry={onRetry} />;
  ...
  return (
    <section id="review-resources" className="panel review-resources-panel">
      <div className="panel-heading">
        ...
        <span>{data.weakPointCount} 个薄弱点</span>
      </div>
      <ModuleResourceMeta resource={resources} onRetry={onRetry} />
      <p className="task-status">更新时间 {new Date(data.generatedAt).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })}，优先处理当前报告和错题本里最容易提分的知识点。</p>
