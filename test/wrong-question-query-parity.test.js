...
test('WrongQuestionQueryService list parity matches legacy stable fields', async () => {
  ...
  const service = new WrongQuestionQueryService(new WrongQuestionProjectionService(createReadOnlyPrisma(facts)));

  const actual = await withDatabaseUrl(() => service.getWrongQuestionsCompat(USER_ID));

  assert.deepEqual(actual.map(pickListFields), legacy.map(pickListFields));
});

test('WrongQuestionQueryService summary parity matches legacy stable counts', async () => {
  const { WrongQuestionQueryService } = require('../apps/api/src/study/wrong-question-query.service.ts');
  const facts = createFacts();
  ...
  try {
    ...
    const actual = await withDatabaseUrl(() => service.getWrongQuestionSummaryCompat(USER_ID));

    assert.deepEqual(pickSummaryCounts(actual), pickSummaryCounts(legacy));
  } finally {
    mock.timers.reset();
  }
});

test('WrongQuestionQueryService due parity matches legacy stable fields', async () => {
  const { WrongQuestionQueryService } = require('../apps/api/src/study/wrong-question-query.service.ts');
  const facts = createFacts();
  const legacyService = createLegacyService(facts);
  const service = new WrongQuestionQueryService(new WrongQuestionProjectionService(createReadOnlyPrisma(facts)));
  ...
  mock.timers.enable({ apis: ['Date'], now: new Date(FIXED_NOW) });
  try {
    const legacy = legacyService.getDueReviews(USER_ID);
    const actual = await withDatabaseUrl(() => service.getDueReviewsCompat(USER_ID));

    assert.deepEqual(actual.items.map(pickDueFields), legacy.items.map(pickDueFields));
  } finally {
    mock.timers.reset();
  }
});
...
function createFacts() {
  ...
  const practiceRecordsForLegacy = [
    practiceRecord({ id: 'r-1', questionId: CURRENT_PENDING_ID, knowledgePointId: CO_POINT_ID, correct: false, mistakeReason: '概念不清', submittedAt: '2026-08-20T08:00:00.000Z' }),
    practiceRecord({ id: 'r-2', questionId: CURRENT_REVIEWED_ID, knowledgePointId: OS_POINT_ID, correct: false, mistakeReason: '审题问题', submittedAt: '2026-08-20T09:00:00.000Z' }),
    practiceRecord({ id: 'r-3', questionId: RESOLVED_ID, knowledgePointId: CO_POINT_ID, correct: false, mistakeReason: null, submittedAt: '2026-08-20T10:00:00.000Z' }),
    practiceRecord({ id: 'r-4', questionId: RESOLVED_ID, knowledgePointId: CO_POINT_ID, correct: true, mistakeReason: null, submittedAt: '2026-08-21T10:00:00.000Z' }),
    practiceRecord({ id: 'r-5', questionId: CURRENT_PENDING_ID, knowledgePointId: CO_POINT_ID, correct: false, mistakeReason: '概念不清', submittedAt: '2026-08-22T08:00:00.000Z' }),
    practiceRecord({ id: 'r-6', questionId: CURRENT_REVIEWED_ID, knowledgePointId: OS_POINT_ID, correct: false, mistakeReason: '审题问题', submittedAt: '2026-08-22T09:00:00.000Z' }),
    practiceRecord({ id: 'r-7', questionId: VARIANT_ID, knowledgePointId: CO_POINT_ID, correct: true, mistakeReason: null, submittedAt: '2026-08-22T11:00:00.000Z', variantQuestionId: CURRENT_PENDING_ID }),
  ];
  const reviewSchedulesForLegacy = [
    reviewSchedule({ questionId: CURRENT_PENDING_ID, stability: 'learning', consecutiveCorrect: 0, nextReviewAt: '2026-08-24T07:00:00.000Z', reviewCount: 1 }),
    reviewSchedule({ questionId: CURRENT_REVIEWED_ID, stability: 'review', consecutiveCorrect: 1, nextReviewAt: '2026-08-24T07:30:00.000Z', reviewCount: 2 }),
    reviewSchedule({ questionId: FUTURE_ID, stability: 'learning', consecutiveCorrect: 0, nextReviewAt: '2026-08-25T07:30:00.000Z', reviewCount: 1 }),
    reviewSchedule({ questionId: MASTERED_ID, stability: 'mastered', consecutiveCorrect: 3, nextReviewAt: '2026-08-23T07:30:00.000Z', reviewCount: 3 }),
  ];
  const knowledgePointsForLegacy = [
    knowledgePoint({ id: CO_POINT_ID, title: 'Cache 映射与替换', subject: '计算机组成原理', chapter: '存储系统', importance: 5 }),
    knowledgePoint({ id: OS_POINT_ID, title: '进程同步与互斥', subject: '操作系统', chapter: '进程管理', importance: 4 }),
  ];
  return {
    questionsForLegacy,
    ...
    wrongQuestionReviews: [
      { questionId: CURRENT_REVIEWED_ID, reviewedAt: new Date('2026-08-22T12:00:00.000Z'), resolved: false, resolvedAt: null },
    ],
...
async function withDatabaseUrl(run) {
  ...
  process.env.DATABASE_URL = 'postgresql://unit-test/wrong-question-query-parity';
  try {
    return await run();
  } finally {
